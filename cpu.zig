...
const Cpu = struct {
    ...
    const SCS = struct {
        ...
        const Regs = struct {
            ...
            const VTOR = packed struct(u32) {
                ...
                inline fn getOffset(self: VTOR) u32 {
                    return @as(u32, @bitCast(self)) & 0x3fff_ff80;
                }
            };
            ...
            const SHPR1 = extern struct {
                const addr = 0xE000ED18;
                pri: [4]u8 align(1),
            };
            const SHPR2 = extern struct {
                const addr = 0xE000ED1C;
                pri: [4]u8 align(1),
            };
            const SHPR3 = extern struct {
            ...
            const NVIC = struct {
                ...
                const NVIC_IPR = extern struct {
                    const addr = 0xE000E400;
                    /// - pri[0] Priority of interrupt number N (0, 4, 8, etc).
                    /// - pri[1] Priority of interrupt number N+1 (1, 5, 9, etc).
                    /// - pri[2] Priority of interrupt number N+2 (2, 6, 10, etc).
                    /// - pri[3] Priority of interrupt number N+3 (3, 7, 11, etc).
                    pri: [4]u8 align(1),
                };
            };
        };
    ...
    const SAM = struct {
        ...
        const Map = struct { //
            const Attr = packed struct(u8) { readable: bool = false, executable: bool = false, writable: bool = false, _1: u5 = 0 };
        ...
        const Layout: [8]Map = .{ //
            code, sram, peripheral, ram0, ram1, device_shared, device_nonshared, system,
        };
    };
    ...
    fn getExeptionPriority(self: *Cpu, except: u9) i16 {
        switch (except) {
            1 => return -3,
            2 => return -2,
            3 => return -1,
            4...7 => |i| {
                const shpr = self.readMemMappedReg(SCS.Regs.SHPR1, 0);
                return shpr.pri[i % 4];
            },
            8...11 => |i| {
                const shpr = self.readMemMappedReg(SCS.Regs.SHPR2, 0);
                return shpr.pri[i % 4];
            },
            12...15 => |i| {
                const shpr = self.readMemMappedReg(SCS.Regs.SHPR3, 0);
                return shpr.pri[i % 4];
            },
            16...511 => |i| {
                const x = @as(packed struct(u9) { //
                    idx: u2,
                    reg: u7,
                }, @bitCast(i - 16));
                const shpr = self.readMemMappedReg(SCS.Regs.NVIC.NVIC_IPR, x.reg);
                return shpr.pri[x.idx];
            },
            else => unreachable,
        }
    }

    const PriorityGroup = struct { //
        group: i16,
        sub: i16,
    };

    fn getPriorityGrouping(self: *Cpu, priority: i16) PriorityGroup {
        if (priority >= 0) {
            const pri: u8 = @intCast(priority);
            const aircr = self.readMemMappedReg(SCS.Regs.AIRCR, 0);
            const sub = @as(u8, pri & (@as(u8, 0b1111_1111) >> (7 - aircr.prigroup)));
    ...
    fn getExecutionPriority(self: *Cpu) i16 {
        ...
        for (2..15) |e| {
            if (self.exception_active[e] == 1) {
                ...
                if (pri.group < highestpri) {
                    highestpri = pri.group;
                }
            }
        }

        if (self.basepri.basepri != 0) {
            boostedpri = self.getPriorityGrouping(self.basepri.basepri).group;
        }
        ...
        if (boostedpri < highestpri) return boostedpri;
        return highestpri;
    }
    ...
    fn returnAddress(self: *Cpu, except: u9, precise: bool) u32 {
        ...
        return switch (exception) {
            ...
            // TODO check precise and imprecise
            .usagefault, .busfault, .memmanage, .hardfault => if (precise) self.current_instr_addr else self.current_instr_addr + incr,
            .debugmonitor => self.current_instr_addr,
            else => |i| if (@intFromEnum(i) >= 16)
                self.PC + incr
            else
                unreachable,
        };
    ...
    fn exceptionEntry(self: *Cpu, ex: u9, precise: bool) !void {
        ...
        if (ex < 16 and expri > -1) {
            ...
            switch (@as(Exception, @enumFromInt(ex))) {
                ...
                .busfault => {
                    reg.busfaultact = true;
                    reg.busfaultpended = false;
                },
                .usagefault => {
                    reg.usgfaultact = true;
                    reg.usgfaultpended = false;
                },
                .svccall => {
                    reg.svccallact = true;
                    reg.svccallpended = false;
                },
                .debugmonitor => {},
                .pendsv => {
                    reg.pendsvact = true;
    ...
    fn getPendingException(self: *Cpu) ?u9 {
        ...
        if (faultreg.memfaultpended) {
            ...
            priority = self.getPriorityGrouping(self.getExeptionPriority(exception.?));
        }

        if (faultreg.busfaultpended) {
            exc = @intFromEnum(Exception.busfault);
        ...
        if (faultreg.usgfaultpended) {
            exc = @intFromEnum(Exception.usagefault);
            pri = self.getPriorityGrouping(self.getExeptionPriority(exc));
    ...
    fn nvicPending(self: *Cpu, intr: u9) bool {
        const x = @as(packed struct(u9) { //
            idx: u5,
            reg: u4,
        }, @bitCast(intr));

        const pendreg = self.readMemMappedReg(SCS.Regs.NVIC.NVIC_ICPR, x.reg);
        const pendreg2 = self.readMemMappedReg(SCS.Regs.NVIC.NVIC_ISPR, x.reg);
        return !pendreg.check(x.idx) and pendreg2.check(x.idx);
    }

    fn nvicEnabled(self: *Cpu, intr: u9) bool {
        const x = @as(packed struct(u9) { //
            idx: u5,
            reg: u4,
        }, @bitCast(intr));

        const pendreg = self.readMemMappedReg(SCS.Regs.NVIC.NVIC_ICER, x.reg);
        const pendreg2 = self.readMemMappedReg(SCS.Regs.NVIC.NVIC_ISER, x.reg);
        return !pendreg.check(x.idx) and pendreg2.check(x.idx);
    }

    fn nvicEnable(self: *Cpu, intr: u9, yes: bool) void {
        const x = @as(packed struct(u9) { //
            idx: u5,
            reg: u4,
        }, @bitCast(intr));
    ...
    fn nvicAtive(self: *Cpu, intr: u9) void {
        ...
        const actreg = self.readMemMappedReg(SCS.Regs.NVIC.NVIC_IABR, x.reg);
        return actreg.check(x.idx);
    }

    fn nvicAtivate(self: *Cpu, intr: u9, yes: bool) void {
        const x = @as(packed struct(u9) { //
            idx: u5,
            reg: u4,
        }, @bitCast(intr));
    ...
    fn deActivate(self: *Cpu, returning_excpt: u9) void {
        ...
        if (returning_excpt > 3 and returning_excpt < 16) {
            switch (@as(Exception, @enumFromInt(returning_excpt))) {
                .memmanage => reg.memfaultact = false,
                .busfault => reg.busfaultact = false,
                .usagefault => reg.usgfaultact = false,
                .svccall => reg.svccallact = false,
                .debugmonitor => {},
                .pendsv => reg.pendsvact = false,
    ...
    fn pushStack(self: *Cpu, execption: u9, precise: bool) !void {
        ...
        const curr_sp = self.regs[curr_sp_index];

        const frameptralign = ((curr_sp & 0b10) > 0) and ccr.stackalign;
        const frameptr = (curr_sp - 0x20) & ~@as(u32, if (ccr.stackalign) 0b100 else 0);

        self.regs[curr_sp_index] = frameptr;

        try self.writeMemA(u32, frameptr, self.getReg(0));
        try self.writeMemA(u32, frameptr + 4, self.getReg(1));
        try self.writeMemA(u32, frameptr + 8, self.getReg(2));
    ...
    fn popStack(self: *Cpu, frameptr: u32, excret: u4) !void {
        self.setReg(0, try self.readMemA(u32, frameptr));
        self.setReg(1, try self.readMemA(u32, frameptr + 4));
        self.setReg(2, try self.readMemA(u32, frameptr + 8));
        self.setReg(3, try self.readMemA(u32, frameptr + 12));
        self.setReg(12, try self.readMemA(u32, frameptr + 16));
        self.setRL(try self.readMemA(u32, frameptr + 20));
        self.setPC(try self.readMemA(u32, frameptr + 24));
        self.psr = @bitCast(try self.readMemA(u32, frameptr + 28));
    ...
    fn exceptionReturn(self: *Cpu, exc_ret: u28) !void {
        ...
        switch (exc_ret & 0b1111) {
            ...
            0b1001, 0b1101 => |v| {
                ...
                self.setMode(.thread);
                self.control.one = if (v == 0b1001) false else true;
            },
            else => {
                self.deActivate(returning_exception_number);
                cfsr.usage_fault.invpc = true;
        ...
        if (self.getMode() == .thread and nested_activation == 0 and scr.sleeponexit) {
            //TODO ??
    ...
    fn resetSCSRegs(self: *Cpu) void {
        //TODO others
        @memset(self.memory[SAM.system.realbegin..][0..std.math.maxInt(u22)], 0);
    }

    fn defaultSCSRegs(self: *Cpu) void {
        //self.writeMemMappedReg(SCS.Regs.VTOR, .{}, 0) catch unreachable;
        @memset(self.memory[SAM.system.realbegin..][0..std.math.maxInt(u22)], 0);
        std.debug.print("================: {}\n", .{self.decoder.endian});
        //TODO others
        //self.mem_steam.seekTo(SAM.system.realbegin) catch unreachable;
    }
    ...
    fn takeReset(self: *Cpu, adjust: bool) !void {
        ...
        const tmp = try self.readMemA(u32, vector_table + 4);
        if (adjust) {
            self.setPC(tmp & 0xffff_fffe);
        } else {
    ...
    fn integerZeroDivideTrappingEnabled(self: *Cpu) bool {
        return (self.readMemMappedReg(SCS.Regs.CCR, 0)).div0trp;
    }
    ...
    fn bxWrtePC(self: *Cpu, addr: u32) !void {
        if (self.getMode() == .handler and (addr & 0xf000_0000) == 0xf000_0000) {
            try self.exceptionReturn(@truncate(addr));
        } else {
            self.psr.t = addr & 1 == 1;
    ...
    fn fetch(self: *Cpu) !Instr {
        ...
        self.on32 = false;
        return self.decoder.decode(hw, false);
    }

    fn checkAlignment(self: *Cpu, T: type, addr: u32) !void {
        if (addr % @sizeOf(T) != 0) {
            var fsr = self.readMemMappedReg(SCS.Regs.CFSR, 0);
    ...
    fn readMemA(self: *Cpu, T: type, addrz: u32) anyerror!T {
        return self.readMemA_withPriv(T, addrz);
    }

    fn readMemA_withPriv(self: *Cpu, T: type, addrz: u32) anyerror!T {
        try self.checkAlignment(T, addrz);
        const addr = try self.translateAddress(addrz, .{ .readable = true });
        return try self.readMemInner(T, addr);
    }

    fn writeMemA(self: *Cpu, T: type, addrz: u32, val: T) anyerror!void {
        try self.writeMemA_withPriv(T, addrz, val);
    }

    fn writeMemA_withPriv(self: *Cpu, T: type, addrz: u32, val: T) anyerror!void {
        try self.checkAlignment(T, addrz);
        const addr = try self.translateAddress(addrz, .{ .writable = true });
        try self.writeMemInner(T, addr, val);
    }

    fn readMemA_Unpriv(self: *Cpu, T: type, addrz: u32) anyerror!T {
        self.checkAlignment(T, addrz);
        const addr = try self.translateAddress(addrz, .{ .readable = true });
        return try self.readMemInner(T, addr);
    }

    fn writeMemA_Unpriv(self: *Cpu, T: type, addrz: u32, val: T) anyerror!void {
        self.checkAlignment(T, addrz);
        const addr = try self.translateAddress(addrz, .{ .writable = true });
        try self.writeMemInner(T, addr, val);
    }

    fn checkUnAlignment(self: *Cpu, T: type, addr: u32) !void {
        if (addr % @sizeOf(T) != 0) {
            const ccr = self.readMemMappedReg(SCS.Regs.CCR, 0);
    ...
    fn readMemU(self: *Cpu, T: type, addrz: u32) anyerror!T {
        return try self.readMemU_withPriv(T, addrz);
    }

    fn readMemU_withPriv(self: *Cpu, T: type, addrz: u32) anyerror!T {
        try self.checkUnAlignment(T, addrz);
        const addr = try self.translateAddress(addrz, .{ .readable = true });
        return try self.readMemInner(T, addr);
    }

    fn writeMemU(self: *Cpu, T: type, addrz: u32, val: T) anyerror!void {
        try self.writeMemU_withPriv(T, addrz, val);
    }

    fn writeMemU_withPriv(self: *Cpu, T: type, addrz: u32, val: T) anyerror!void {
        try self.checkUnAlignment(T, addrz);
        const addr = try self.translateAddress(addrz, .{ .writable = true });
        try self.writeMemInner(T, addr, val);
    }

    fn readMemU_Unpriv(self: *Cpu, T: type, addrz: u32) anyerror!T {
        try self.checkUnAlignment(T, addrz);
        const addr = try self.translateAddress(addrz, .{ .readable = true });
        return try self.readMemInner(T, addr);
    }

    fn writeMemU_Unpriv(self: *Cpu, T: type, addrz: u32, val: T) anyerror!void {
        try self.checkUnAlignment(T, addrz);
        const addr = try self.translateAddress(addrz, .{ .writable = true });
        try self.writeMemInner(T, addr, val);
    }
            ...
            for (0..8) |i| {
                if (bits & 1 == 1) {
                    if (i == a.n and i != lwbs) {
                        ...
                        try self.writeMemA(u32, address, self.getReg(i));
                    }
                    address += 4;
                }
                bits >>= 1;
    ...
    fn ldrlitT1(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            const data = try self.readMemU(u32, addr);
    ...
    fn bxT1(self: *Cpu) !void {
        ...
        if (self.conditionPassed()) {
            ...
            try self.bxWrtePC(self.getReg(a.m));
        }
    }
    ...
    fn strimmT2(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            try self.writeMemU(u32, self.getReg(13) + (@as(u32, a.imm) << 2), self.getReg(a.t));
        }
    }
    ...
    fn ldrhimmT1(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            self.setReg(a.rt, try self.readMemU(u16, addr));
        }
    }
    ...
    fn strhimmT1(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            try self.writeMemU(u16, self.getReg(a.rn) + (@as(u32, a.imm5) << 1), @truncate(self.getReg(a.rt)));
        }
    }
    ...
    fn ldrbimmT1(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            self.setReg(a.rt, try self.readMemU(u8, self.getReg(a.rn) + a.imm));
        }
    }
    ...
    fn strbimmT1(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            try self.writeMemU(u8, self.getReg(a.rn) + a.imm, @truncate(self.getReg(a.rt)));
        }
    }
    ...
    fn strimmT1(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            try self.writeMemU(u32, self.getReg(a.n) + (@as(u32, a.imm) << 2), self.getReg(a.t));
        }
    }
    ...
    fn ldrhregT1(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            self.setReg(a.rt, try self.readMemU(u16, self.getReg(a.rn) + self.getReg(a.rm)));
        }
    }
    ...
    fn ldrbregT1(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            self.setReg(a.rt, try self.readMemU(u8, self.getReg(a.rn) + self.getReg(a.rm)));
        }
    }
    ...
    fn ldrshregT1(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            self.setReg(a.rt, @bitCast(@as(i32, @intCast(@as(i16, @bitCast(try self.readMemU(u16, self.getReg(a.rn) + self.getReg(a.rm))))))));
        }
    }
    ...
    fn ldrsbregT1(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            self.setReg(a.rt, @bitCast(@as(i32, @intCast(@as(i8, @bitCast(try self.readMemU(u8, self.getReg(a.rn) + self.getReg(a.rm))))))));
        }
    }
    ...
    fn strbregT1(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            try self.writeMemU(u8, self.getReg(a.n) + self.getReg(a.m), @truncate(self.getReg(a.t)));
        }
    }
    ...
    fn strhregT1(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            try self.writeMemU(u16, self.getReg(a.rn) + self.getReg(a.rm), @truncate(self.getReg(a.rt)));
        }
    }
    ...
    fn strregT1(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            try self.writeMemU(u32, self.getReg(a.rn) + self.getReg(a.rm), self.getReg(a.rt));
        }
    }
    ...
    fn pushT1(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            for (0..8) |i| {
                if (a & 1 > 0) {
                    try self.writeMemA(u32, addr, self.getReg(i));
                    addr += 4;
                }
                a >>= 1;
            }
    ...
    fn cps(self: *Cpu) void {
        if (self.currentModeIsPrivileged()) {
            ...
            if (!a.disable) {
                ...
                if (a.affectfault and self.getExecutionPriority() > -1) self.faultmask.fm = true;
            }
        }
    }
    ...
    fn pushT3(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            try self.writeMemA(u32, address, self.getReg(a.rt));
            self.setReg(SP_REG, address);
        }
    }
    ...
    fn strdimmT1(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            try self.writeMemA(u32, addr, self.getReg(a.rt));
            try self.writeMemA(u32, addr + 4, self.getReg(a.rt2));
    ...
    fn ldrdimmT1(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            self.setReg(a.rt, try self.readMemA(u32, addr));
            self.setReg(a.rt2, try self.readMemA(u32, addr + 4));
    ...
    fn strexbT1(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            if (self.exclusiveMonitorsPass(address, 1)) {
                try self.writeMemA(u8, address, @truncate(self.getReg(a.rt)));
                self.setReg(a.rd, 0);
            } else {
                self.setReg(a.rd, 1);
            }
    ...
    fn strexhT1(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            if (self.exclusiveMonitorsPass(address, 2)) {
                try self.writeMemA(u16, address, @truncate(self.getReg(a.rt)));
                self.setReg(a.rd, 0);
            } else {
                self.setReg(a.rd, 1);
            }
    ...
    fn tbbT1(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            const half_words = if (a.H)
                try self.readMemA(u16, self.getReg(a.rn + std.math.shl(u32, self.getReg(a.rm), 1)))
            else
                @as(u16, try self.readMemA(u8, self.getReg(a.rn + self.getReg(a.rm))));
    ...
    fn ldrexbT1(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            self.setReg(a.rt, try self.readMemA(u8, address));
        }
    }
    ...
    fn ldrexhT1(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            self.setReg(a.rt, try self.readMemA(u16, address));
        }
    }
    ...
    fn ldrregT2(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            const data = try self.readMemU(u32, address);
    ...
    fn ldrhimmT3(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            const offset_addr = if (add) self.getReg(a.rn) + a.imm8 else self.getReg(a.rn) - a.imm8;
            const addr = if (index) offset_addr else self.getReg(a.rn);
            const data = try self.readMemU(u16, addr);
            if (wback) self.setReg(a.rn, offset_addr);
            self.setReg(a.rt, data);
        }
    }
    ...
    fn ldrhtT1(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            const data = try self.readMemU_Unpriv(u16, addr);
    ...
    fn ldrtT1(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            const addr = self.getReg(a.rn) + a.imm8;
            const data = try self.readMemU_Unpriv(u32, addr);
    ...
    fn ldrhregT2(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            const offset = shift32(self.getReg(a.rm), .lsl, a.imm2, self.psr.c);
            const address = self.getReg(a.rn) + offset;
            const data = try self.readMemU(u16, address);
            self.setReg(a.rt, data);
        }
    }
    ...
    fn ldrbimmT2(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            const addr = self.getReg(a.rn) + a.imm12;
            self.setReg(a.rt, try self.readMemU(u8, addr));
        }
    }
    ...
    fn ldrbimmT3(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            const offset_addr = if (add) self.getReg(a.rn) + a.imm8 else self.getReg(a.rn) - a.imm8;
            const addr = if (index) offset_addr else self.getReg(a.rn);
            const data = try self.readMemU(u8, addr);
            self.setReg(a.rt, data);
            if (wback) self.setReg(a.rn, offset_addr);
        }
    }
    ...
    fn ldrbtT1(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            const addr = self.getReg(a.rn) + a.imm8;
            const data = try self.readMemU_Unpriv(u8, addr);
    ...
    fn ldrbregT2(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            const offset = shift32(self.getReg(a.rm), .lsl, a.imm2, self.psr.c);
            const address = self.getReg(a.rn) + offset;
            const data = try self.readMemU(u8, address);
            self.setReg(a.rt, data);
        }
    }
    ...
    fn strbimmT2(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            const addr = self.getReg(a.rn) + a.imm12;
            try self.writeMemU(u8, addr, @truncate(self.getReg(a.rt)));
        }
    }
    ...
    fn strbregT2(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            const offset = shift32(self.getReg(a.rm), .lsl, a.imm2, self.psr.c);
            const address = self.getReg(a.rn) + offset;
            try self.writeMemU(u8, address, @truncate(self.getReg(a.rt)));
        }
    }
    ...
    fn strhimmT2(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            const addr = self.getReg(a.rn) + a.imm12;
            try self.writeMemU(u16, addr, @truncate(self.getReg(a.rt)));
        }
    }
    ...
    fn strhregT2(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            const offset = shift32(self.getReg(a.rm), .lsl, a.imm2, self.psr.c);
            const address = self.getReg(a.rn) + offset;
            try self.writeMemU(u16, address, @truncate(self.getReg(a.rt)));
        }
    }
    ...
    fn strimmT3(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            const addr = self.getReg(a.rn) + a.imm12;
            try self.writeMemU(u32, addr, self.getReg(a.rt));
        }
    }
    ...
    fn strregT2(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            const offset = shift32(self.getReg(a.rm), .lsl, a.imm2, self.psr.c);
            const address = self.getReg(a.rn) + offset;
            try self.writeMemU(u32, address, self.getReg(a.rt));
        }
    }
    ...
    fn strbt(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            try self.writeMemU_Unpriv(u8, self.getReg(a.rn) + a.imm8, @truncate(self.getReg(a.rt)));
        }
    }
    ...
    fn strt(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            try self.writeMemU_Unpriv(u32, self.getReg(a.rn) + a.imm8, @truncate(self.getReg(a.rt)));
        }
    }
    ...
    fn strht(self: *Cpu) !void {
        if (self.conditionPassed()) {
            ...
            try self.writeMemU_Unpriv(u16, self.getReg(a.rn) + a.imm8, @truncate(self.getReg(a.rt)));
        }
    }
    ...
    fn exec(self: *Cpu, instr: Instr) !void {
        ...
        switch (instr) {
            //else => unreachable,
            .strt => try self.strt(),
            .strht => try self.strht(),
            .strbt => try self.strbt(),
            .unpredictable => {},
            .rorimmT1 => self.rorimmT1(),
            .ldrsbimmT2 => try self.ldrsbimmT2(),
            .ldrblitT1 => try self.ldrblitT1(),
            .sbfxT1 => self.sbfxT1(),
            .ldrshimmT2 => try self.ldrshimmT2(),
            .ldrtT1 => try self.ldrtT1(),
            .bT1 => self.bT1(),
            .svc => try self.svcT1(),
            .unknown => try self.undefined(),
            .umlalT1 => self.umlalT1(),
            .smlalT1 => self.smlalT1(),
            .udivT1 => try self.udivT1(),
            .umullT1 => self.umullT1(),
            .sdivT1 => try self.sdivT1(),
            ...
            .strregT2 => try self.strregT2(),
            .strimmT4 => try self.strimmT4(),
            .strimmT3 => try self.strimmT3(),
            .strhregT2 => try self.strhregT2(),
            .strhimmT2 => try self.strhimmT2(),
            .strhimmT3 => try self.strhimmT3(),
            .strbregT2 => try self.strbregT2(),
            .strbimmT2 => try self.strbimmT2(),
            .strbimmT3 => try self.strbimmT3(),
            .pldimmT1 => {}, // we dont do that here
            .ldrsbregT2 => try self.ldrsbregT2(),
            .ldrsblitT1 => try self.ldrsblitT1(),
            .ldrsbtT1 => try self.ldrsbtT1(),
            .ldrsbimmT1 => try self.ldrsbimmT1(),
            .ldrbregT2 => try self.ldrbregT2(),
            .ldrbtT1 => try self.ldrbtT1(),
            .ldrbimmT3 => try self.ldrbimmT3(),
            .ldrbimmT2 => try self.ldrbimmT2(),
            .ldrshregT2 => try self.ldrshregT2(),
            .ldrshlitT1 => try self.ldrshlitT1(),
            .ldrshtT1 => try self.ldrshtT1(),
            .ldrshimmT1 => try self.ldrshimmT1(),
            .ldrhregT2 => try self.ldrhregT2(),
            .ldrhlitT1 => try self.ldrhlitT1(),
            .ldrhtT1 => try self.ldrhtT1(),
            .ldrhimmT3 => try self.ldrhimmT3(),
            .ldrhimmT2 => try self.ldrhimmT2(),
            .ldrlitT2 => try self.ldrlitT2(),
            .ldrregT2 => try self.ldrregT2(),
            .ldrimmT4 => try self.ldrimmT4(),
            .ldrimmT3 => try self.ldrimmT3(),
            .ldrexhT1 => try self.ldrexhT1(),
            .ldrexbT1 => try self.ldrexbT1(),
            .tbbT1 => try self.tbbT1(),
            .strexhT1 => try self.strexhT1(),
            .strexbT1 => try self.strexbT1(),
            .strdimmT1 => try self.strdimmT1(),
            .ldrdimmT1 => try self.ldrdimmT1(),
            .ldrdlitT1 => try self.ldrdlitT1(),
            .ldrexT1 => try self.ldrexT1(),
            .strexT1 => try self.strexT1(),
            .ldmdbT1 => try self.ldmdbT1(),
            //TODO pushT3
            .pushT2 => try self.pushT2(),
            .stmdbT1 => try self.stmdbT1(),
            //TODO popT3
            .popT2 => try self.popT2(),
            .ldmT2 => try self.ldmT2(),
            .stmT2 => try self.stmT2(),
            .isbT1 => self.isb(),
            .dmbT1 => self.dmb(),
            .dsbT1 => self.dsb(),
            .clrexT1 => self.clrex(),
            ...
            //====================
            .ldmT1 => try self.ldmT1(),
            .stmT1 => try self.stmT1(),
            .addspimmT1 => self.addspimmT1(),
            .adrT1 => self.adrT1(),
            .ldrlitT1 => try self.ldrlitT1(),
            ...
            .ldrimmT2 => try ldrimmT2(self),
            .strimmT2 => try strimmT2(self),
            .ldrhimmT1 => try ldrhimmT1(self),
            .strhimmT1 => try strhimmT1(self),
            .ldrbimmT1 => try ldrbimmT1(self),
            .strbimmT1 => try strbimmT1(self),
            .ldrimmT1 => try ldrimmT1(self),
            .strimmT1 => try strimmT1(self),
            .ldrregT1 => try ldrregT1(self),
            .ldrhregT1 => try ldrhregT1(self),
            .ldrbregT1 => try ldrbregT1(self),
            .ldrshregT1 => try ldrshregT1(self),
            .ldrsbregT1 => try ldrsbregT1(self),
            .strbregT1 => try strbregT1(self),
            .strhregT1 => try strhregT1(self),
            .strregT1 => try strregT1(self),
            .popT1 => try popT1(self),
            .revshT1 => revshT1(self),
            .rev16T1 => rev16T1(self),
            .revT1 => revT1(self),
            .pushT1 => try pushT1(self),
            .uxtbT1 => uxtbT1(self),
            .uxthT1 => uxthT1(self),
            .sxtbT1 => sxtbT1(self),
            .sxthT1 => sxthT1(self),
    ...
    inline fn incrPC(self: *Cpu) void {
        if (self.on32) {
            self.PC += 4;
        } else {
            self.PC += 2;
...
pub const Decoder = struct {
    ...
    fn instr32(self: *Decoder, word: u32) Instr {
        const wl: u16 = @truncate(word);
        const wh: u16 = @truncate(word >> 16);
        //@as(u32, self.getWord() catch unreachable);
        self.current = word;
